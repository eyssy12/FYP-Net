(function () {
    'use strict';

    var apiBase = '/api/classes';
    var getClassForStudent = '/getClassForStudent';

    angular
        .module(angularAppName)
        .factory('ClassService', ClassService);

    ClassService.$inject = ['HttpService'];

    function ClassService(HttpService) {
        var service = {};

        service.get = function () {
            return HttpService.get(apiBase);
        }

        service.post = function (data) {
            return HttpService.post(apiBase, data);
        }

        service.put = function (id, data) {
            return HttpService.put(apiBase + '/', id, data);
        }

        service.delete = function (id) {
            return HttpService.delete(apiBase + '/', id);
        }

        service.bulkDelete = function (idArrayViewModel) {
            return HttpService.bulkDelete(apiBase, idArrayViewModel);
        }

        service.getClassForStudent = function () {
            return HttpService.get(apiBase + getClassForStudent);
        };

        return service;
    }
})();